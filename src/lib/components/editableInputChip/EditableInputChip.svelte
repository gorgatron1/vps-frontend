<script lang="ts">
	...
	const cChipList = 'flex flex-wrap gap-0';
	...
	const onEdit = (v: string, i: number) => {
		value[i] = v;
	};
	...
	const onReorder = (i: number) => {
		if (null === dragged) return;
		if (i - dragged === 1) return;
		const _v = value.splice(dragged, 1);
		const newI = dragged < i ? i - 1 : i;
		value.splice(newI, 0, ..._v);
		value = value;
		dragged = null;
		hovering = null;
	};
...
<div class="input-chip {classesBase}" class:opacity-50={$$restProps.disabled}>
	...
	<div class="input-chip-interface {classesInterface}">
		...
		{#if chipValues.length}
			<div class="input-chip-list {classesChipList} -mx-2">
				{#each chipValues as { id, val }, i (id)}
					<!-- Wrapping div required for FLIP animation -->
					<div class="flex my-1">
						<div
							class="w-4 -mx-1 h-full z-10"
							class:bg-secondary-400={hovering === i}
							on:dragenter={() => (hovering = i)}
							on:dragleave={() => (hovering = null)}
							on:drop|preventDefault={() => onReorder(i)}
